...
io.on('connection', (socket) => {
    ...
    socket.on('claim-task', (data, cb) => {
        try {
            ...
            const task = getPlayerTaskStmt.get(playerId, taskId);
            const now = new Date().toISOString();
            if (!task) {
                insertPlayerTaskStmt.run(playerId, taskId, 'completed', now);
            } else {
                updatePlayerTaskStmt.run('completed', now, playerId, taskId);
            }

            // Выдать награду
            const taskInfo = db.prepare('SELECT reward FROM daily_tasks WHERE id = ?').get(taskId);
            if (taskInfo) {
                const player = players.find(p => p.id === playerId);
                if (player) {
                    player.resources += taskInfo.reward;
                    upsertPlayerStmt.run({
                        id: player.id,
                        name: player.name,
                        resources: player.resources,
