...
const onlineUsers = new Set(); // Stores usernames of connected players
...
wss.on('connection', (ws, req) => {
  ...
  users[username] = { ws, online: true, lastSeen: Date.now() };
  onlineUsers.add(username); // Add to online users set
  ...
  ws.on('message', (message) => {
    try {
      ...
      if (data.type === 'register') {
        // Already registered via URL params, just acknowledge
        ws.send(JSON.stringify({
          type: 'registered',
          username: username
        }));
    ...
    // Check if a user exists (online only)
    if (data.type === 'check_user') {
      const username = data.username;
      const exists = onlineUsers.has(username); // or database check
      
      ws.send(JSON.stringify({
        type: 'user_check',
        username: username, // Actual value, not template
        exists
      }));
    }
    ...
    if (currentUser) {
      ...
      switch (data.type) {
        case 'friend_request':
          ...
          if ((friendships.friends[username] || []).includes(to)) {
              ws.send(JSON.stringify({
                  type: 'error',
          ...
          const existingRequest = friendships.requests.find(
              r => (r.from === username && r.to === to) || 
                  (r.from === to && r.to === username)
          );
          
          if (existingRequest) {
              ws.send(JSON.stringify({
                  type: 'error',
                  message: existingRequest.from === username ? 
