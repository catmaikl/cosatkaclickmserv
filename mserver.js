...
function handleMessage(data, player) {
  switch (data.type) {
    ...
    case 'player_ready':
      ...
      const readyRoom = rooms.get(player.roomId);
      if (!readyRoom) {
        player.ws.send(JSON.stringify({
          type: 'error',
      ...
      // Проверяем, готов ли второй игрок
      const otherPlayerId = readyRoom.player1 === player.id ? readyRoom.player2 : readyRoom.player1;
      const otherPlayer = players.get(otherPlayerId);

      if (otherPlayer && otherPlayer.ready) {
        // Оба игрока готовы - начинаем баттл
        startBattle(readyRoom);
      } else {
...
app.post('/battle/chat', (req, res) => {
  const { roomId, username, message } = req.body;
  
  if (!battleRooms[roomId] || !battleRooms[roomId].players.includes(username)) {
    return res.status(400).json({ error: 'Неверная комната или пользователь' });
  }
  ...
  battleRooms[roomId].chat.push(chatMessage);
  
  // Отправляем сообщение обоим игрокам
  battleRooms[roomId].players.forEach(player => {
    if (users[player] && users[player].ws) {
      users[player].ws.send(JSON.stringify({
