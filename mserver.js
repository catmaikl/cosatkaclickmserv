...
function startBattle(player1, player2) {
    ...
    // Notify both players
    players[player1].send(JSON.stringify({
        type: 'battle_start',
        battleId,
    ...
    players[player2].send(JSON.stringify({
        type: 'battle_start',
        battleId,
    ...
    const timer = setInterval(() => {
        ...
        Object.keys(battle.players).forEach(playerId => {
            if (players[playerId]) {
                players[playerId].send(JSON.stringify({
                    type: 'battle_update',
                    timeLeft: battle.timeLeft,
                    yourScore: battle.scores[playerId],
                    opponentScore: battle.scores[playerId === player1 ? player2 : player1],
                    yourCps: battle.cps[playerId],
                    opponentCps: battle.cps[playerId === player1 ? player2 : player1]
                }));
            }
        });
...
function endBattle(battleId) {
    ...
    const winner = battle.scores[player1] > battle.scores[player2] ? player1 : 
                  battle.scores[player1] < battle.scores[player2] ? player2 : null;
