...
WEBHOOK_URL = os.environ.get('WEBHOOK_URL', '')  # Будет установлен в Render
...
def handle_messages(message):
    try:
        ...
        # Всегда регистрируем пользователя при первом обращении
    ...
    try:
        ...
        if call.data == 'buy_miner':
            ...
            if success:
                ...
                bot.answer_callback_query(call.id, f"❌ Недостаточно монets! Нужно: {cost}")
...
if __name__ == '__main__':
    try:
        ...
        if WEBHOOK_URL:
            ...
            print("Webhook URL не установлен, используем polling")
