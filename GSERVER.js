...
async function grantPremiumSkinForWin(userId) {
  try {
    ...
    if (availableSkins.length === 0) {
      return null; // Все скины уже получены
    }
    ...
    if (!selectedSkin) {
      selectedSkin = availableSkins[0]; // Fallback
    }
...
// API для получения премиум скинов пользователя
...
// API для сохранения разблокированного скина
app.post("/api/user/unlock-skin", authenticateToken, async (req, res) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      db.run(
        `INSERT OR IGNORE INTO user_premium_skins (user_id, skin_id, obtained_from) 
         VALUES (?, ?, ?)`,
