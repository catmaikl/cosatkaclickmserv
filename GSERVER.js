...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      db.run(`INSERT INTO users (id, username, email, password_hash) 
              VALUES (?, ?, ?, ?)`,
...
app.get("/api/user/profile", authenticateToken, async (req, res) => {
  try {
    const user = await new Promise((resolve, reject) => {
      db.get(`SELECT id, username, email, score, level, cps, total_clicks, 
                     created_at, last_login, last_updated 
              FROM users WHERE id = ?`, [req.user.userId], (err, row) => {
        if (err) reject(err);
        else resolve(row);
      });
    });
...
app.post("/api/leaderboard", authenticateToken, async (req, res) => {
  try {
    const { score, level, cps, totalClicks } = req.body;
    const userId = req.user.userId;

    // Обновляем данные пользователя
    await new Promise((resolve, reject) => {
      db.run(`UPDATE users SET score = ?, level = ?, cps = ?, total_clicks = ?,
                     last_updated = CURRENT_TIMESTAMP 
              WHERE id = ?`,
        [score, level, cps, totalClicks, userId], (err) => {
          if (err) reject(err);
          else resolve();
        });
    });
    ...
    // Обновляем лидерборд
    await new Promise((resolve, reject) => {
      db.run(`INSERT OR REPLACE INTO leaderboard (user_id, username, score, level, cps, last_updated)
              SELECT id, username, score, level, cps, last_updated 
              FROM users WHERE id = ?`,
        [userId], (err) => {
          if (err) reject(err);
          else resolve();
...
// Добавьте эндпоинт для получения топ-N игроков
