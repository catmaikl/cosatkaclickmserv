...
// Добавляем новые обработчики маршрутов
app.post('/api/unlock-skill', async (req, res) => {
  ...
  try {
    ...
    // Проверяем, есть ли уже такой навык у игрока
    if (player.unlockedSkills.includes(skillId)) {
      return res.status(400).json({ error: 'Skill already unlocked' });
    }
    
    // Проверяем, есть ли навык в списке доступных для разблокировки
    ...
    if (player.gold < skill.unlockCost) {
      return res.status(400).json({ error: 'Not enough gold' });
    }
    
    // Разблокируем навык
    player.unlockedSkills.push(skillId);
    player.gold -= skill.unlockCost;
    await player.save();
...
app.post('/api/buy-item', async (req, res) => {
  ...
  try {
    ...
    // Проверяем уровень игрока, если у предмета есть требование
...
io.on('connection', (socket) => {
  ...
  // Обработчик разблокировки навыка
  socket.on('unlock_skill', async ({ skillId }) => {
    try {
      const player = await Player.findOne({ username: socket.username });
      if (!player) return;
      ...
      const skill = await Skill.findById(skillId);
      if (!skill) {
        return socket.emit('error', { message: 'Skill not found' });
      }
      
      // Проверяем требования
      if (player.unlockedSkills.includes(skillId)) {
        return socket.emit('error', { message: 'Skill already unlocked' });
      }
      
      if (skill.requiredLevel > player.level) {
        return socket.emit('error', { message: 'Level requirement not met' });
      }
      
      if (player.gold < skill.unlockCost) {
        return socket.emit('error', { message: 'Not enough gold' });
      }
      ...
      // Оповещаем других игроков (если нужно)
  ...
  socket.on('buy_item', async ({ itemId, quantity = 1 }) => {
    try {
      const player = await Player.findOne({ username: socket.username });
      if (!player) return;
      ...
      if (!item.availableInShop) {
        return socket.emit('error', { message: 'Item not available for purchase' });
      }
...
async function initializeGlobalEvents() {
  try {
    // Use timeout and retry for initial query
    currentGlobalEvent = await GlobalEvent.findOne({ active: true })
      .maxTimeMS(10000)
      .catch(() => null); // Silently fail and create new event
    
    if (!currentGlobalEvent) {
      console.log('No active global event found, creating new one');
      await createGlobalEvent();
    } else {
      console.log('Found active global event:', currentGlobalEvent.name);
    ...
    console.error('Global event initialization error:', err);
    // Fallback to creating new event
    await createGlobalEvent().catch(e => console.error('Failed to create fallback event:', e));
  }
}
...
async function startServer() {
  ...
  server.listen(PORT, () => {
    ...
    // Initialize events after server starts
    initializeGlobalEvents().catch(console.error);
  });
}
